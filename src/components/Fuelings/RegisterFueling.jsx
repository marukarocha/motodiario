// src/components/Fuelings/RegisterFueling.jsx
import React, { useState, useContext } from 'react'; // Importe useContext
import { Form, Button } from 'react-bootstrap';
import Swal from 'sweetalert2';
import { addFueling } from '../DB/firebaseServices';
import { useAuth } from '../USER/Auth/AuthContext'; // Importe useAuth
...
function RegisterFueling({ onClose }) {
    ...
    const postos = ['Posto A', 'Posto B', 'Posto C']; // Substitua pelos seus postos
    const combustiveis = ['Gasolina', 'Etanol', 'Diesel']; // Substitua pelos seus combustíveis
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            Swal.fire({
                title: 'Sucesso!',
                html: `Abastecimento registrado com sucesso. <br/> <a href="/listar-abastecimento">Ver lista de abastecimentos</a>`, // Use HTML no Swal
                icon: 'success',
                confirmButtonText: 'Fechar'
            }).then((result) => {
                if (result.isConfirmed) {
                    onClose(); // Fecha o modal após a confirmação
                    setData({ // Limpa o formulário
                        data: new Date().toLocaleDateString(),
                        hora: new Date().toLocaleTimeString(),
                        posto: '',
                        combustivel: '',
